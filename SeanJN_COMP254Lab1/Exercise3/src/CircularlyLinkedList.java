...
public class CircularlyLinkedList<E> implements Cloneable {
  ...
  public CircularlyLinkedList<E> clone() throws CloneNotSupportedException {
    ...
    if (size > 0) {
      Node<E> head = tail.getNext(); // Since CircularlyLinkedLists doesn't track the head

      Node<E> otherHead = new Node<>(head.getElement(), null); // Set the head to the other list
      Node<E> otherTail = otherHead;
